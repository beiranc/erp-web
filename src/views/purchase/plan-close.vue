<template>
  <el-row :gutter="24">
    <el-col :span="24">
      <el-divider content-position="center">关闭采购计划</el-divider>
        ...
        <el-table :data="purchaseData" style="width: 100%">
          ...
          <el-table-column label="操作" align="center" width="200">
            <template slot-scope="scope">
              ...
              <el-popconfirm confirm-button-text="确认" cancel-button-text="取消" icon="el-icon-delete" icon-color="red" title="确认执行此操作？" @onConfirm="closePurchase(scope.row)">
                <el-button slot="reference" v-permission="['purchaseManager', 'purchase:edit']" size="mini" type="danger" :disabled="scope.row.purchaseState === 'CLOSED'">关闭</el-button>
              </el-popconfirm>
            </template>
          </el-table-column>
...
import { getPurchases, closePurchase } from '@/api/purchase/index'
...
export default {
  ...
  methods: {
    ...
    closePurchase(row) {
      // 关闭采购计划
      closePurchase(row.purchaseId).then(res => {
        this.$message({
          type: 'success',
          message: '操作成功'
        })
